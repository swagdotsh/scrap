...
Settings contains only the persistent scrobbling on/off switch and **Retry pending scrobbles**. Account controls are below the main pages. Click the tray icon to reopen Scrap, or right-click it for **Open Scrap** and **Quit Scrap**. If Windows cannot create the tray icon, closing exits normally so the app cannot become inaccessible.
